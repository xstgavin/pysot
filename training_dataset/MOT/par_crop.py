...
def crop_json(json_elm, sub_set_crop_path,img_root, instanc_size=511):
    ...
    imgElms=json_elm[0].split('/')
    frame_crop_base_path = join(sub_set_crop_path, imgElms[0]+'/'+imgElms[1]+'/')
    ...
    for id, object_iter in enumerate(json_elm[1]):
        ...
        obj_class = object_iter['class']
        frame_idx = object_iter['frame']
        object_id = object_iter['id']
        visibility = object_iter['visibility']
        if not isdir(frame_crop_base_path): makedirs(frame_crop_base_path)
        if visibility<0.3:
           continue
        z, x = crop_like_SiamFC(im, bbox, instanc_size=instanc_size, padding=avg_chans)
        cv2.imwrite(join(frame_crop_base_path, '{:06d}.{:02d}.z.jpg'.format(frame_idx, object_id)), z)
        cv2.imwrite(join(frame_crop_base_path, '{:06d}.{:02d}.x.jpg'.format(frame_idx, object_id)), x)

def main(instanc_size=511, num_threads=24):
    crop_path = './MOT2017/crop{:d}'.format(instanc_size)
    if not isdir(crop_path): mkdir(crop_path)
    VID_base_path = './MOT2017/'
    sub_sets=['train','test']
    for sub_set in sub_sets:
        ...
        #jdata_tmp={}
        #for k_idx, key in enumerate( jdata.keys()):
        #   if k_idx>20:
        #      break
        #   jdata_tmp[key] = jdata[key].copy()

        sub_set_crop_path = crop_path
        with futures.ProcessPoolExecutor(max_workers=num_threads) as executor:
            fs = [executor.submit(crop_json, json_elm, sub_set_crop_path, './MOT2017/'+sub_set,instanc_size) for  json_elm  in jdata.items()]
